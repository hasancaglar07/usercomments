...
export async function generateMetadata(
  ...
  const description =
    review.metaDescription ?? review.excerpt ?? "Read the full review and user experiences.";
...
export default async function Page(props: PageProps) {
  ...
  try {
    ...
    const [fetchedComments, fetchedCategories] = await Promise.all([
      review
  ...
  const reviewJsonLd = {
    ...
    itemReviewed: {
      "@type": "Product",
      name: review.title,
      image: productImg ? [toAbsoluteUrl(productImg)] : undefined,
    },
    reviewRating:
      ratingAvg > 0
        ? {
      ...
      <div className="w-full flex justify-center py-6 px-4 md:px-10 lg:px-20 bg-background-light dark:bg-background-dark">
        <div className="layout-content-container w-full max-w-7xl">
          ...
          <nav className="flex flex-wrap gap-2 text-sm text-slate-500 dark:text-slate-400 mb-6" aria-label="Breadcrumb">
            <Link href={localizePath("/", lang)} className="hover:text-primary transition-colors">Home</Link>
            <span className="text-slate-300">/</span>
            <Link href={localizePath("/catalog", lang)} className="hover:text-primary transition-colors">Catalog</Link>
            {categoryLabel && (
              <>
                <span className="text-slate-300">/</span>
                <Link
