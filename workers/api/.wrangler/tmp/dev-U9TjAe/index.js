...
// .wrangler/tmp/bundle-LsDNLa/strip-cf-connecting-ip-header.js
...
var envSchema = external_exports.object({
  ...
  B2_S3_ENDPOINT: external_exports.string().min(1).optional(),
  B2_S3_REGION: external_exports.string().min(1).optional(),
  B2_S3_ACCESS_KEY_ID: external_exports.string().min(1).optional(),
  B2_S3_SECRET_ACCESS_KEY: external_exports.string().min(1).optional(),
  B2_S3_BUCKET: external_exports.string().min(1).optional(),
  B2_PUBLIC_BASE_URL: external_exports.string().min(1).optional(),
  RATE_LIMIT_WINDOW_SEC: external_exports.coerce.number().int().positive().default(60),
  RATE_LIMIT_MAX: external_exports.coerce.number().int().positive().default(60),
  CACHE_TTL_CATEGORIES_SEC: external_exports.coerce.number().int().positive().default(21600),
  CACHE_TTL_SUBCATEGORIES_SEC: external_exports.coerce.number().int().positive().default(21600),
  CACHE_TTL_LATEST_SEC: external_exports.coerce.number().int().positive().default(45),
  CACHE_TTL_POPULAR_SEC: external_exports.coerce.number().int().positive().default(45),
  CACHE_TTL_REVIEW_LIST_SEC: external_exports.coerce.number().int().positive().default(60),
  CACHE_TTL_REVIEW_SEC: external_exports.coerce.number().int().positive().default(30),
  CACHE_TTL_REVIEW_COMMENTS_SEC: external_exports.coerce.number().int().positive().default(30),
  CACHE_TTL_USER_SEC: external_exports.coerce.number().int().positive().default(90),
  CACHE_TTL_USER_REVIEWS_SEC: external_exports.coerce.number().int().positive().default(90),
  CACHE_TTL_SEARCH_SEC: external_exports.coerce.number().int().positive().default(30),
  CACHE_TTL_SITEMAP_SEC: external_exports.coerce.number().int().positive().default(1800)
});
var parsedEnv = null;
function getEnv(env) {
  if (!parsedEnv) {
...
__name(mapCategoryRow, "mapCategoryRow");
function mapProfileRow(row) {
  return {
    username: row.username,
    displayName: row.username,
    bio: row.bio ?? void 0,
    profilePicUrl: row.profile_pic_url ?? void 0,
    createdAt: row.created_at ?? void 0
  };
}
__name(mapProfileRow, "mapProfileRow");
function mapReviewRow(row, options) {
  const profile = pickRelation(row.profiles);
  const translations = Array.isArray(row.review_translations) ? row.review_translations.filter(
    (item) => Boolean(item)
  ).map((translation) => ({
    lang: translation.lang,
    slug: translation.slug,
    title: translation.title,
    contentHtml: translation.content_html ?? void 0,
    metaTitle: translation.meta_title ?? void 0,
    metaDescription: translation.meta_description ?? void 0
  })) : [];
  const preferredTranslation = options?.lang ? translations.find((translation) => translation.lang === options.lang) : translations[0];
  const photoUrls = Array.isArray(row.photo_urls) ? row.photo_urls.filter((item) => typeof item === "string") : void 0;
  ...
  return {
    ...
    excerpt: row.excerpt ?? "",
    contentHtml: preferredTranslation?.contentHtml ?? row.content_html ?? void 0,
    metaTitle: preferredTranslation?.metaTitle,
    metaDescription: preferredTranslation?.metaDescription,
    translations: options?.includeTranslations ? translations.map((translation) => ({
      lang: translation.lang,
      slug: translation.slug
    })) : void 0,
    ratingAvg,
    ...
    author: {
      ...
      displayName: profile?.username ?? void 0,
      profilePicUrl: profile?.profile_pic_url ?? void 0
    },
    createdAt: row.created_at,
    categoryId: categoryId ?? void 0,
    subCategoryId: subCategoryId ?? void 0
  };
}
__name(mapReviewRow, "mapReviewRow");
...
__name(pickRelation, "pickRelation");

// src/services/profiles.ts
var ROLE_VALUES = ["user", "moderator", "admin"];
function normalizeUsername(value) {
  return value.toLowerCase().replace(/[^a-z0-9_-]/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "").slice(0, 24);
}
...
__name(buildBaseUsername, "buildBaseUsername");
async function ensureProfileForUser(env, user) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("user_id").eq("user_id", user.id).maybeSingle();
  if (error) {
    throw error;
  }
  if (data) {
  ...
  for (const username of candidates) {
    const { error: insertError } = await supabase2.from("profiles").insert({
      user_id: user.id,
      username
    });
    if (!insertError) {
    ...
    const { data: existing, error: existingError } = await supabase2.from("profiles").select("user_id").eq("user_id", user.id).maybeSingle();
    if (existingError) {
      throw existingError;
    }
    if (existing) {
  ...
  const { error: fallbackError } = await supabase2.from("profiles").insert({
    user_id: user.id,
    username: fallback
  });
  if (fallbackError) {
...
__name(ensureProfileForUser, "ensureProfileForUser");
async function fetchProfileRole(env, userId) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("role").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
...
__name(fetchProfileRole, "fetchProfileRole");
async function fetchProfileByUserId(env, userId) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("user_id, username").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
__name(fetchProfileByUserId, "fetchProfileByUserId");
async function fetchProfileDetailsByUserId(env, userId) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("user_id, username, bio, profile_pic_url, created_at").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
__name(fetchProfileDetailsByUserId, "fetchProfileDetailsByUserId");
async function updateProfileByUserId(env, userId, payload) {
  const supabase2 = getSupabaseClient(env);
  const updates = {};
  ...
  if (payload.profilePicUrl !== void 0) {
    updates.profile_pic_url = payload.profilePicUrl;
  }
  const { data, error } = await supabase2.from("profiles").update(updates).eq("user_id", userId).select("user_id, username, bio, profile_pic_url, created_at").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
__name(buildCacheKeyRequest, "buildCacheKeyRequest");
function buildCacheKeyFromUrl(url) {
  return buildCacheKeyRequest(new Request(url));
}
__name(buildCacheKeyFromUrl, "buildCacheKeyFromUrl");
async function purgeCacheUrls(urls) {
  const cache = caches.default ?? caches;
  let deleted = 0;
...
__name(purgeCacheUrls, "purgeCacheUrls");
async function cacheResponse(request, ctx, ttlSeconds, handler) {
...
function cleanupBuckets(now, windowSec) {
...
function buildPaginationInfo(page, pageSize, totalItems) {
  ...
  return {
    ...
    pageSize,
    totalPages,
    totalItems: totalItems ?? void 0
  };
}
__name(buildPaginationInfo, "buildPaginationInfo");

// src/services/categories.ts
var categorySelect = "id, name, parent_id, category_translations(lang, name)";
function mapCategoryWithTranslations(row, lang) {
  const translation = Array.isArray(row.category_translations) ? row.category_translations.find((item) => item.lang === lang) : null;
  return {
    id: row.id,
...
__name(mapCategoryWithTranslations, "mapCategoryWithTranslations");
async function fetchCategories(env, lang) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("categories").select(categorySelect).order("id");
  if (error) {
    throw error;
  }
  return (data ?? []).map((row) => mapCategoryWithTranslations(row, lang));
}
__name(fetchCategories, "fetchCategories");
async function fetchSubcategories(env, parentId, lang) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("categories").select(categorySelect).eq("parent_id", parentId).order("id");
  if (error) {
    throw error;
  }
  return (data ?? []).map((row) => mapCategoryWithTranslations(row, lang));
}
__name(fetchSubcategories, "fetchSubcategories");
async function createCategory(env, payload) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("categories").insert({
    name: payload.name,
    parent_id: payload.parentId ?? null
  }).select(categorySelect).single();
  if (error || !data) {
    throw error;
  }
...
async function updateCategory(env, id, payload) {
  const supabase2 = getSupabaseClient(env);
  const updates = {};
  ...
  const { data, error } = await supabase2.from("categories").update(updates).eq("id", id).select(categorySelect).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
// src/utils/i18n.ts
var SUPPORTED_LANGUAGES = ["tr", "en", "es", "de", "ar"];
...
var HIDDEN_REVIEW_TITLE = "Review not available";
var HIDDEN_REVIEW_EXCERPT = "This review is currently unavailable.";
var reviewListSelect = `
  id,
  slug,
  title,
  excerpt,
...
`;
var reviewDetailSelect = `
  ${reviewListSelect},
  content_html,
  status
`;
var reviewTranslationSelect = `
...
`;
var reviewListSelectWithTranslations = `
  ${reviewListSelect},
  ${reviewTranslationSelect}
`;
async function fetchPopularReviews(env, limit, lang) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select(reviewListSelectWithTranslations).eq("review_translations.lang", lang).eq("status", "published").order("votes_up", { ascending: false }).order("views", { ascending: false }).limit(limit);
  if (error) {
    throw error;
  }
  return (data ?? []).map((row) => mapReviewRow(row, { lang }));
}
__name(fetchPopularReviews, "fetchPopularReviews");
async function fetchLatestReviews(env, cursor, limit, lang) {
  const supabase2 = getSupabaseClient(env);
  let query = supabase2.from("reviews").select(reviewListSelectWithTranslations).eq("review_translations.lang", lang).eq("status", "published").order("created_at", { ascending: false }).limit(limit);
  if (cursor) {
  ...
  const items = (data ?? []).map((row) => mapReviewRow(row, { lang }));
  const nextCursor = items.length > 0 ? items[items.length - 1].createdAt : null;
  return { items, nextCursor };
}
__name(fetchLatestReviews, "fetchLatestReviews");
async function fetchReviews(env, options) {
  const supabase2 = getSupabaseClient(env);
  const { categoryId, subCategoryId, sort, page, pageSize, lang } = options;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("reviews").select(reviewListSelectWithTranslations, { count: "exact" }).eq("review_translations.lang", lang);
  if (categoryId) {
  ...
  return {
    items: (data ?? []).map((row) => mapReviewRow(row, { lang })),
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
  };
}
__name(fetchReviews, "fetchReviews");
async function fetchReviewBySlug(env, slug, lang) {
  const supabase2 = getSupabaseClient(env);
  ...
  `;
  const { data, error } = await supabase2.from("review_translations").select(translationSelect).eq("lang", lang).eq("slug", slug).maybeSingle();
  if (error) {
    throw error;
  }
  let translation = data;
  if (!translation) {
    const { data: lookup, error: lookupError } = await supabase2.from("review_translations").select("review_id").eq("slug", slug).maybeSingle();
    if (lookupError) {
      throw lookupError;
    }
    if (!lookup?.review_id) {
    ...
    const { data: fallbackTranslation, error: fallbackError } = await supabase2.from("review_translations").select(translationSelect).eq("review_id", lookup.review_id).eq("lang", DEFAULT_LANGUAGE).maybeSingle();
    if (fallbackError) {
      throw fallbackError;
    }
  ...
  const translationList = Array.isArray(reviewRow.review_translations) ? reviewRow.review_translations.filter(
    (item) => Boolean(item) && isSupportedLanguage(item.lang)
  ) : [];
  const mergedReview = {
    ...reviewRow,
    review_translations: [
      ...
      ...translationList.filter((item) => item.lang !== translation.lang).map((item) => ({
        lang: item.lang,
        slug: item.slug,
        title: ""
      }))
...
__name(fetchReviewBySlug, "fetchReviewBySlug");
async function fetchReviewMetaById(env, id) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select("slug, category_id, status, profiles(username), review_translations(lang, slug)").eq("id", id).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  const translations = Array.isArray(data.review_translations) ? data.review_translations.filter(
    (item) => Boolean(item) && isSupportedLanguage(item.lang)
  ) : [];
  ...
  return {
    ...
    authorUsername: Array.isArray(data.profiles) ? data.profiles[0]?.username ?? null : data.profiles?.username ?? null,
    translations: translations.length > 0 ? translations : void 0
  };
}
__name(fetchReviewMetaById, "fetchReviewMetaById");
async function fetchCommentStatusById(env, id) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("comments").select("id, review_id, status").eq("id", id).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
__name(fetchCommentStatusById, "fetchCommentStatusById");
async function fetchReviewComments(env, reviewId, cursor, limit) {
  const supabase2 = getSupabaseClient(env);
  let query = supabase2.from("comments").select("id, review_id, text, created_at, profiles(username, profile_pic_url)").eq("review_id", reviewId).eq("status", "published").order("created_at", { ascending: false }).limit(limit);
  if (cursor) {
    query = query.lt("created_at", cursor);
  }
  const { data, error } = await query;
...
__name(fetchReviewComments, "fetchReviewComments");
async function createReview(env, payload) {
  ...
  const { data, error } = await supabase2.from("reviews").insert({
    slug,
    title: payload.title,
...
async function addComment(env, payload) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("comments").insert({
    review_id: payload.reviewId,
    user_id: payload.userId,
    text: payload.text
  }).select("id, review_id, text, created_at, profiles(username, profile_pic_url)").single();
  if (error || !data) {
    throw error;
  }
...
async function addVote(env, payload) {
  const supabase2 = getSupabaseClient(env);
  const { error: voteError } = await supabase2.from("review_votes").insert({
    review_id: payload.reviewId,
  ...
  const { data: reviewData, error: reviewError } = await supabase2.from("reviews").select("votes_up, votes_down").eq("id", payload.reviewId).single();
  if (reviewError || !reviewData) {
    throw reviewError;
  }
  const votesUp = (reviewData.votes_up ?? 0) + (payload.type === "up" ? 1 : 0);
  const votesDown = (reviewData.votes_down ?? 0) + (payload.type === "down" ? 1 : 0);
  const { error: updateError } = await supabase2.from("reviews").update({ votes_up: votesUp, votes_down: votesDown }).eq("id", payload.reviewId);
  if (updateError) {
    throw updateError;
  }
  return { votesUp, votesDown };
}
__name(addVote, "addVote");
async function incrementReviewViews(env, reviewId) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.rpc("increment_review_view", {
    review_id: reviewId
  });
  if (error) {
    throw error;
...
__name(incrementReviewViews, "incrementReviewViews");
async function fetchReviewsByUserComments(env, userId, page, pageSize, lang) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.rpc("get_user_commented_reviews", {
  ...
  const rows = data ?? [];
  const reviewIds = rows.map((row) => row.id).filter((id) => typeof id === "string" && id.length > 0);
  ...
  const items = rows.map(
    (row) => mapReviewRow(attachTranslation(row, translations.get(row.id)), {
      lang
    })
  );
...
__name(fetchReviewsByUserComments, "fetchReviewsByUserComments");
async function fetchSavedReviews(env, userId, page, pageSize, lang) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase2.from("saved_reviews").select(
  ...
  const rows = (data ?? []).map((row) => row.reviews).filter((review) => Boolean(review));
  const translations = await fetchReviewTranslationMap(
    env,
  ...
  const items = rows.map(
    (review) => mapReviewRow(attachTranslation(review, translations.get(review.id)), { lang })
  );
  return {
    items,
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
...
__name(fetchSavedReviews, "fetchSavedReviews");
async function fetchReviewsByUserId(env, userId, page, pageSize, lang, statuses = ["published"]) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("reviews").select(reviewListSelectWithTranslations, { count: "exact" }).eq("user_id", userId).eq("review_translations.lang", lang).order("created_at", { ascending: false });
...
__name(fetchReviewsByUserId, "fetchReviewsByUserId");
async function fetchReviewTranslationMap(env, reviewIds, lang) {
  if (reviewIds.length === 0) {
    return /* @__PURE__ */ new Map();
  }
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("review_translations").select("review_id, lang, slug, title, content_html, meta_title, meta_description").in("review_id", reviewIds).eq("lang", lang);
  if (error) {
    throw error;
  }
...
__name(fetchReviewTranslationMap, "fetchReviewTranslationMap");
function attachTranslation(review, translation) {
  if (!translation) {
    return review;
  }
  return {
...
__name(attachTranslation, "attachTranslation");
...
__name(buildHiddenReview, "buildHiddenReview");
...
__name(normalizeTotalCount, "normalizeTotalCount");
async function ensureUniqueSlug(env, baseSlug) {
  const supabase2 = getSupabaseClient(env);
  let candidate = baseSlug;
  let suffix = 1;
  while (true) {
    const { data, error } = await supabase2.from("reviews").select("id").eq("slug", candidate).maybeSingle();
    if (error) {
      throw error;
    }
    if (!data) {
...
__name(ensureUniqueSlug, "ensureUniqueSlug");

// src/services/search.ts
async function searchReviews(env, options) {
  const supabase2 = getSupabaseClient(env);
  ...
  let query = supabase2.from("review_translations").select(
    `
        review_id,
        lang,
        slug,
      ...
      `,
    { count: "exact" }
  ).eq("lang", lang).eq("reviews.status", "published").or(`title.ilike.${pattern},content_html.ilike.${pattern}`);
  if (categoryId) {
...
__name(searchReviews, "searchReviews");

// src/services/users.ts
async function fetchUserProfileRecord(env, username) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("user_id, username, bio, profile_pic_url, created_at").eq("username", username).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  const { data: statsData, error: statsError } = await supabase2.rpc(
    "get_user_stats",
  ...
  const profile = {
    ...baseProfile,
    stats: {
      reviewCount: statsRow && Number.isFinite(Number(statsRow.review_count)) ? Number(statsRow.review_count) : void 0,
      totalViews: statsRow && Number.isFinite(Number(statsRow.total_views)) ? Number(statsRow.total_views) : void 0,
      reputation: statsRow && Number.isFinite(Number(statsRow.total_votes)) ? Number(statsRow.total_votes) : void 0,
      karma: statsRow && Number.isFinite(Number(statsRow.total_votes)) ? Number(statsRow.total_votes) : void 0,
      totalComments: statsRow && Number.isFinite(Number(statsRow.total_comments)) ? Number(statsRow.total_comments) : void 0
    }
  };
  return {
    userId: data.user_id,
...
__name(fetchUserProfileRecord, "fetchUserProfileRecord");
...
async function createPresignedUploadUrl(env, userId, filename, contentType) {
...
__name(createPresignedUploadUrl, "createPresignedUploadUrl");

// src/services/reports.ts
function pickRelation2(value) {
  if (Array.isArray(value)) {
    return value[0] ?? null;
  }
  return value ?? null;
}
__name(pickRelation2, "pickRelation");
function normalizeReviewStatus(value) {
  if (value === "hidden" || value === "deleted" || value === "pending" || value === "draft") {
    return value;
  }
  return "published";
}
__name(normalizeReviewStatus, "normalizeReviewStatus");
...
__name(normalizeCommentStatus, "normalizeCommentStatus");
...
__name(normalizeUserRole, "normalizeUserRole");
...
__name(mapReportRow, "mapReportRow");
async function createReport(env, payload) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reports").insert({
    reporter_user_id: payload.reporterUserId,
    target_type: payload.targetType,
    target_id: payload.targetId,
    reason: payload.reason,
    details: payload.details ?? null
  }).select("id, reporter_user_id, target_type, target_id, reason, details, created_at, status").single();
  if (error || !data) {
    throw error;
  }
...
async function fetchReports(env, options) {
  const supabase2 = getSupabaseClient(env);
  const { status, page, pageSize } = options;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("reports").select(
  ...
  const baseItems = (data ?? []).map((row) => mapReportRow(row));
  const reviewIds = baseItems.filter((item) => item.targetType === "review").map((item) => item.targetId);
  const commentIds = baseItems.filter((item) => item.targetType === "comment").map((item) => item.targetId);
  const userIds = baseItems.filter((item) => item.targetType === "user").map((item) => item.targetId);
  const [reviewResult, commentResult, userResult] = await Promise.all([
    reviewIds.length ? supabase2.from("reviews").select("id, slug, title, status, profiles(username)").in("id", reviewIds) : Promise.resolve({ data: [], error: null }),
    commentIds.length ? supabase2.from("comments").select("id, review_id, text, status, profiles(username), reviews(slug, title, status)").in("id", commentIds) : Promise.resolve({ data: [], error: null }),
    userIds.length ? supabase2.from("profiles").select("user_id, username, role, profile_pic_url, bio").in("user_id", userIds) : Promise.resolve({ data: [], error: null })
  ]);
  ...
  (reviewResult.data ?? []).forEach((row) => {
    const reviewRow = row;
    const profile = pickRelation2(reviewRow.profiles);
    reviewMap.set(reviewRow.id, {
      id: reviewRow.id,
      slug: reviewRow.slug,
      title: reviewRow.title ?? "",
      status: normalizeReviewStatus(reviewRow.status),
      author: profile?.username ? { username: profile.username } : void 0
    });
  });
  const commentMap = /* @__PURE__ */ new Map();
  (commentResult.data ?? []).forEach((row) => {
    const commentRow = row;
    const profile = pickRelation2(commentRow.profiles);
    const review = pickRelation2(commentRow.reviews);
    commentMap.set(commentRow.id, {
      ...
      reviewTitle: review?.title ?? void 0,
      author: profile?.username ? { username: profile.username } : void 0
    });
  });
  ...
  (userResult.data ?? []).forEach((row) => {
    ...
    userMap.set(userRow.user_id, {
      ...
      profilePicUrl: userRow.profile_pic_url ?? void 0,
      bio: userRow.bio ?? void 0
    });
  });
...
__name(fetchReports, "fetchReports");
async function updateReportStatus(env, id, status) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reports").update({ status }).eq("id", id).select("id, reporter_user_id, target_type, target_id, reason, details, created_at, status").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
var adminReviewSelect = `
  ...
  profiles(username, profile_pic_url)
`;
var adminReviewDetailSelect = `
  ${adminReviewSelect},
  content_html
`;
var adminCommentSelect = `
  id,
  review_id,
  text,
  status,
...
function pickRelation3(value) {
  if (Array.isArray(value)) {
    return value[0] ?? null;
  }
  return value ?? null;
}
__name(pickRelation3, "pickRelation");
function normalizeReviewStatus2(value) {
  if (value === "hidden" || value === "deleted" || value === "pending" || value === "draft") {
    return value;
  }
  return "published";
}
__name(normalizeReviewStatus2, "normalizeReviewStatus");
function normalizeCommentStatus2(value) {
  if (value === "hidden" || value === "deleted") {
    return value;
  }
  return "published";
}
__name(normalizeCommentStatus2, "normalizeCommentStatus");
function normalizeUserRole2(value) {
  if (value === "admin" || value === "moderator") {
    return value;
  }
  return "user";
}
__name(normalizeUserRole2, "normalizeUserRole");
function mapAdminReviewRow(row) {
  const profile = pickRelation3(row.profiles);
  const ratingAvg = normalizeNumber2(row.rating_avg);
  const ratingCount = normalizeNumber2(row.rating_count);
  const views = normalizeNumber2(row.views);
  const votesUp = normalizeNumber2(row.votes_up);
  const votesDown = normalizeNumber2(row.votes_down);
  const photoCount = normalizeNumber2(row.photo_count);
  const commentCount = normalizeNumber2(row.comment_count);
  const categoryId = normalizeNumber2(row.category_id);
  const subCategoryId = normalizeNumber2(row.sub_category_id);
  ...
  return {
    ...
    title: row.title,
    excerpt: row.excerpt ?? "",
    contentHtml: row.content_html ?? void 0,
    status: normalizeReviewStatus2(row.status),
    createdAt: row.created_at,
...
__name(mapAdminReviewRow, "mapAdminReviewRow");
function mapAdminCommentRow(row) {
  const profile = pickRelation3(row.profiles);
  const review = pickRelation3(row.reviews);
  return {
    id: row.id,
    reviewId: row.review_id,
    text: row.text ?? "",
    status: normalizeCommentStatus2(row.status),
    createdAt: row.created_at,
    author: {
      username: profile?.username ?? "unknown",
      displayName: profile?.username ?? void 0,
    ...
    review: review ? {
      ...
      slug: review.slug,
      title: review.title ?? "",
      status: normalizeReviewStatus2(review.status)
    } : void 0
  };
}
__name(mapAdminCommentRow, "mapAdminCommentRow");
function mapAdminUserRow(row) {
  return {
    userId: row.user_id,
    username: row.username,
    role: normalizeUserRole2(row.role),
    createdAt: row.created_at ?? void 0
  };
}
__name(mapAdminUserRow, "mapAdminUserRow");
...
__name(mapAdminUserDetailRow, "mapAdminUserDetailRow");
async function fetchAdminReviews(env, options) {
  const supabase2 = getSupabaseClient(env);
  const { status, page, pageSize } = options;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("reviews").select(adminReviewSelect, { count: "exact" }).order("created_at", { ascending: false });
  if (status) {
    query = query.eq("status", status);
  }
  const { data, error, count } = await query.range(from, to);
  ...
  return {
    items: (data ?? []).map((row) => mapAdminReviewRow(row)),
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
  };
}
__name(fetchAdminReviews, "fetchAdminReviews");
async function fetchAdminReviewDetail(env, id) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select(adminReviewDetailSelect).eq("id", id).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  return mapAdminReviewRow(data);
}
__name(fetchAdminReviewDetail, "fetchAdminReviewDetail");
async function updateAdminReview(env, id, payload) {
  const supabase2 = getSupabaseClient(env);
  const updates = {};
  if (payload.title !== void 0) {
    updates.title = payload.title;
  }
  if (payload.excerpt !== void 0) {
    updates.excerpt = payload.excerpt;
  }
  if (payload.contentHtml !== void 0) {
    updates.content_html = payload.contentHtml;
  }
  if (payload.photoUrls !== void 0) {
  ...
  if (payload.subCategoryId !== void 0) {
    updates.sub_category_id = payload.subCategoryId;
  }
  const { data, error } = await supabase2.from("reviews").update(updates).eq("id", id).select(adminReviewDetailSelect).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
__name(updateAdminReview, "updateAdminReview");
async function fetchAdminComments(env, options) {
  const supabase2 = getSupabaseClient(env);
  const { status, reviewId, page, pageSize } = options;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("comments").select(adminCommentSelect, { count: "exact" }).order("created_at", { ascending: false });
  if (status) {
    query = query.eq("status", status);
  }
  if (reviewId) {
  ...
  return {
    items: (data ?? []).map((row) => mapAdminCommentRow(row)),
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
  };
}
__name(fetchAdminComments, "fetchAdminComments");
async function updateAdminComment(env, id, payload) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("comments").update({ text: payload.text }).eq("id", id).select(adminCommentSelect).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  return mapAdminCommentRow(data);
}
__name(updateAdminComment, "updateAdminComment");
async function fetchAdminUsers(env, options) {
  const supabase2 = getSupabaseClient(env);
  const { page, pageSize } = options;
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase2.from("profiles").select("user_id, username, role, created_at", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
  if (error) {
    throw error;
  }
  return {
    items: (data ?? []).map((row) => mapAdminUserRow(row)),
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
  };
}
__name(fetchAdminUsers, "fetchAdminUsers");
async function fetchAdminUserDetail(env, userId) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").select("user_id, username, role, bio, profile_pic_url, created_at").eq("user_id", userId).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
  ...
  return mapAdminUserDetailRow(data);
}
__name(fetchAdminUserDetail, "fetchAdminUserDetail");

// src/services/moderation.ts
async function updateReviewStatus(env, id, status) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").update({ status }).eq("id", id).select("id, slug, status").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function updateCommentStatus(env, id, status) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("comments").update({ status }).eq("id", id).select("id, review_id, status").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function updateUserRole(env, userId, role) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("profiles").update({ role }).eq("user_id", userId).select("user_id, role").maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function fetchSitemapReviews(env, page, pageSize, lang) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase2.from("reviews").select("created_at, updated_at, review_translations!inner(lang, slug)", {
    count: "exact"
  }).eq("review_translations.lang", lang).eq("status", "published").order("created_at", { ascending: false }).range(from, to);
  if (error) {
    throw error;
  }
  const items = (data ?? []).map((row) => {
    const translations = Array.isArray(row.review_translations) ? row.review_translations : [];
...
__name(fetchSitemapReviews, "fetchSitemapReviews");
async function fetchSitemapReviewCount(env, lang) {
  const supabase2 = getSupabaseClient(env);
  const { count, error } = await supabase2.from("reviews").select("id, review_translations!inner(lang)", { count: "exact", head: true }).eq("review_translations.lang", lang).eq("status", "published");
  if (error) {
    throw error;
  }
  return count ?? 0;
}
__name(fetchSitemapReviewCount, "fetchSitemapReviewCount");
async function fetchSitemapCategories(env, lang) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("categories").select("id, name").order("id");
  if (error) {
    throw error;
  }
  const items = (data ?? []).map((row) => ({
...
__name(fetchSitemapCategories, "fetchSitemapCategories");

// src/index.ts
var MAX_SITEMAP_PAGE_SIZE = 5e4;
...
var limitSchema = external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE);
var langSchema = external_exports.string().optional().transform((value) => normalizeLanguage(value));
var cursorSchema = external_exports.string().optional().refine((value) => !value || !Number.isNaN(Date.parse(value)), {
  message: "cursor must be a valid date string"
});
var popularQuerySchema = external_exports.object({
  limit: limitSchema,
  lang: langSchema
});
var latestQuerySchema = external_exports.object({
  cursor: cursorSchema,
  limit: limitSchema,
  lang: langSchema
});
var listQuerySchema = external_exports.object({
  categoryId: external_exports.coerce.number().int().positive().optional(),
  subCategoryId: external_exports.coerce.number().int().positive().optional(),
  sort: external_exports.enum(["latest", "popular", "rating"]).optional(),
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE),
  lang: langSchema
});
var slugParamSchema = external_exports.object({
  slug: external_exports.string().min(1)
});
var reviewIdParamSchema = external_exports.object({
  id: external_exports.string().uuid()
});
var commentIdParamSchema = external_exports.object({
  id: external_exports.string().uuid()
});
var userIdParamSchema = external_exports.object({
  userId: external_exports.string().uuid()
});
var commentQuerySchema = external_exports.object({
  cursor: cursorSchema,
  limit: limitSchema
});
var createReviewSchema = external_exports.object({
  title: external_exports.string().min(3),
  excerpt: external_exports.string().min(10),
  contentHtml: external_exports.string().min(10),
  rating: external_exports.number().min(0).max(5),
  categoryId: external_exports.number().int().positive(),
  subCategoryId: external_exports.number().int().positive().optional(),
  photoUrls: external_exports.array(external_exports.string().url()).default([])
});
...
var voteSchema = external_exports.object({
  type: external_exports.enum(["up", "down"]).default("up")
});
var reportBodySchema = external_exports.object({
  reason: external_exports.string().min(3).max(200),
  details: external_exports.string().max(1e3).optional()
});
var reportQuerySchema = external_exports.object({
  status: external_exports.enum(["open", "resolved", "rejected"]).optional(),
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE)
});
var reportStatusSchema = external_exports.object({
  status: external_exports.enum(["resolved", "rejected"])
});
var reviewStatusSchema = external_exports.object({
  status: external_exports.enum(["published", "hidden", "deleted", "pending", "draft"])
});
var commentStatusSchema = external_exports.object({
  status: external_exports.enum(["published", "hidden", "deleted"])
});
var userRoleSchema = external_exports.object({
  role: external_exports.enum(["user", "moderator", "admin"])
});
var adminReviewQuerySchema = external_exports.object({
  status: external_exports.enum(["published", "hidden", "deleted", "pending", "draft"]).optional(),
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE)
});
var adminReviewUpdateSchema = external_exports.object({
  title: external_exports.string().trim().min(3).max(160).optional(),
  excerpt: external_exports.string().trim().min(10).max(300).optional(),
  contentHtml: external_exports.string().min(10).optional(),
  photoUrls: external_exports.array(external_exports.string().url()).optional(),
  categoryId: external_exports.coerce.number().int().positive().optional(),
  subCategoryId: external_exports.coerce.number().int().positive().nullable().optional()
}).refine(
  (value) => value.title !== void 0 || value.excerpt !== void 0 || value.contentHtml !== void 0 || value.photoUrls !== void 0 || value.categoryId !== void 0 || value.subCategoryId !== void 0,
  "At least one field is required."
);
var adminCommentQuerySchema = external_exports.object({
  status: external_exports.enum(["published", "hidden", "deleted"]).optional(),
  reviewId: external_exports.string().uuid().optional(),
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE)
});
var adminCommentUpdateSchema = external_exports.object({
  text: external_exports.string().trim().min(1).max(2e3)
});
var adminUserQuerySchema = external_exports.object({
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE)
});
var userListQuerySchema = external_exports.object({
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE),
  lang: langSchema
});
var adminCategoryCreateSchema = external_exports.object({
  name: external_exports.string().trim().min(2).max(60),
  parentId: external_exports.number().int().positive().nullable().optional()
});
var adminCategoryUpdateSchema = external_exports.object({
  name: external_exports.string().trim().min(2).max(60).optional(),
  parentId: external_exports.number().int().positive().nullable().optional()
}).refine(
  (value) => value.name !== void 0 || value.parentId !== void 0,
  "At least one field is required."
);
var profileUpdateSchema = external_exports.object({
  username: external_exports.string().trim().min(3).max(24).regex(/^[a-z0-9_-]+$/i, "username must be alphanumeric with - or _.").optional(),
  bio: external_exports.string().trim().max(280).optional().nullable(),
  profilePicUrl: external_exports.string().url().optional().nullable()
}).refine(
  (value) => value.username !== void 0 || value.bio !== void 0 || value.profilePicUrl !== void 0,
  "At least one field is required."
);
var searchQuerySchema = external_exports.object({
  q: external_exports.string().trim().optional().default(""),
  categoryId: external_exports.coerce.number().int().positive().optional(),
  page: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_PAGE_SIZE).default(DEFAULT_PAGE_SIZE),
  lang: langSchema
});
var presignSchema = external_exports.object({
  filename: external_exports.string().min(1),
  contentType: external_exports.string().min(1)
});
var sitemapReviewsQuerySchema = external_exports.object({
  part: external_exports.coerce.number().int().positive().default(1),
  pageSize: external_exports.coerce.number().int().positive().max(MAX_SITEMAP_PAGE_SIZE).default(5e3),
  lang: langSchema
});
...
function errorResponse(status, message, details, headers) {
...
__name(getQueryObject, "getQueryObject");
...
function throwRateLimit(retryAfter) {
  throw new HttpError(429, "Too Many Requests", void 0, {
    "Retry-After": String(retryAfter)
  });
}
__name(throwRateLimit, "throwRateLimit");
function enforceRateLimit(request, env, userId) {
...
var POPULAR_LIMITS = [3, 4, 6];
var LATEST_LIMITS = [3];
var DEFAULT_PAGE = 1;
var DEFAULT_PAGE_SIZE_CACHE = 10;
function buildApiUrl(origin, path, params) {
...
function buildReviewCacheUrls(options) {
  ...
  const translationEntries = translations && translations.length > 0 ? translations : slug ? [{ lang: DEFAULT_LANGUAGE, slug }] : [];
...
__name(buildReviewCacheUrls, "buildReviewCacheUrls");
function buildCategoryCacheUrls(origin, parentId) {
  const urls = [];
...
__name(buildCategoryCacheUrls, "buildCategoryCacheUrls");
function queueCachePurge(ctx, urls) {
  if (urls.length === 0) {
    return;
  }
  ctx.waitUntil(purgeCacheUrls(urls));
}
__name(queueCachePurge, "queueCachePurge");
async function handleHealth() {
  return jsonResponse({ ok: true, timestamp: (/* @__PURE__ */ new Date()).toISOString() }, {
    headers: { "Cache-Control": "no-store" }
  });
}
__name(handleHealth, "handleHealth");
async function handleCategories({ env, url }) {
  const { lang } = external_exports.object({ lang: langSchema }).parse(getQueryObject(url));
...
async function handleSubcategories({
  ...
  const { id } = external_exports.object({ id: external_exports.coerce.number().int().positive() }).parse(params);
  const { lang } = external_exports.object({ lang: langSchema }).parse(getQueryObject(url));
...
__name(handleSubcategories, "handleSubcategories");
async function handlePopularReviews({ env, url }) {
...
__name(handlePopularReviews, "handlePopularReviews");
async function handleLatestReviews({ env, url }) {
...
__name(handleLatestReviews, "handleLatestReviews");
async function handleReviewsList({ env, url }) {
  const { categoryId, subCategoryId, sort, page, pageSize, lang } = listQuerySchema.parse(getQueryObject(url));
...
__name(handleReviewsList, "handleReviewsList");
async function handleReviewBySlug({ env, params, url }) {
  const { lang } = external_exports.object({ lang: langSchema }).parse(getQueryObject(url));
...
__name(handleReviewBySlug, "handleReviewBySlug");
async function handleReviewComments({ env, params, url }) {
...
__name(handleReviewComments, "handleReviewComments");
async function handleReviewView({ env, params, request }) {
  const { id } = reviewIdParamSchema.parse(params);
...
__name(handleReviewView, "handleReviewView");
async function handleCreateReview({ env, request }) {
...
async function handleReportReview({ env, request, params }) {
  const user = await requireAuth(request, env);
  enforceRateLimit(request, env, user.id);
  const { id } = reviewIdParamSchema.parse(params);
...
async function handleReportComment({ env, request, params }) {
  const user = await requireAuth(request, env);
  enforceRateLimit(request, env, user.id);
...
__name(handleReportComment, "handleReportComment");
async function handleReportUser({ env, request, params }) {
  const user = await requireAuth(request, env);
  enforceRateLimit(request, env, user.id);
...
__name(handleReportUser, "handleReportUser");
async function handleUserProfile({ env, params }) {
  const { username } = external_exports.object({ username: external_exports.string().min(1) }).parse(params);
...
async function handleUserReviews({ env, params, url }) {
  const { username } = external_exports.object({ username: external_exports.string().min(1) }).parse(params);
...
__name(handleUserReviews, "handleUserReviews");
async function handleUserComments({ env, params, url }) {
  const { username } = external_exports.object({ username: external_exports.string().min(1) }).parse(params);
  const { page, pageSize, lang } = userListQuerySchema.parse(getQueryObject(url));
  const record = await fetchUserProfileRecord(env, username);
  if (!record) {
...
__name(handleUserComments, "handleUserComments");
...
__name(handleUserDrafts, "handleUserDrafts");
...
__name(handleUserSaved, "handleUserSaved");
...
async function handlePresign({ env, request }) {
  const user = await requireAuth(request, env);
  enforceRateLimit(request, env, user.id);
...
async function handleSitemapReviewsJson({ env, url }) {
...
__name(handleSitemapReviewsJson, "handleSitemapReviewsJson");
async function handleSitemapCategoriesJson({ env, url }) {
  const { lang } = external_exports.object({ lang: langSchema }).parse(getQueryObject(url));
  const result = await fetchSitemapCategories(env, lang);
  return jsonResponse(result);
}
__name(handleSitemapCategoriesJson, "handleSitemapCategoriesJson");
async function handleSitemapIndexXml({ env, request }) {
  const origin = new URL(request.url).origin;
  const { lang } = external_exports.object({ lang: langSchema }).parse(
...
__name(handleSitemapIndexXml, "handleSitemapIndexXml");
async function handleSitemapCategoriesXml({ env, request }) {
  const origin = new URL(request.url).origin;
  const { lang } = external_exports.object({ lang: langSchema }).parse(
    getQueryObject(new URL(request.url))
  );
...
__name(handleSitemapCategoriesXml, "handleSitemapCategoriesXml");
async function handleSitemapReviewsXml({ env, request, url }) {
  const origin = new URL(request.url).origin;
  const { part, pageSize, lang } = sitemapReviewsQuerySchema.parse(getQueryObject(url));
  const result = await fetchSitemapReviews(env, part, pageSize, lang);
...
__name(handleSitemapReviewsXml, "handleSitemapReviewsXml");
async function handleAdminCategories({ env, request }) {
  const user = await requireAuth(request, env);
...
__name(handleAdminCategories, "handleAdminCategories");
...
__name(handleAdminCategoryCreate, "handleAdminCategoryCreate");
async function handleAdminCategoryUpdate({
  ...
  if (payload.parentId !== void 0 && payload.parentId === id) {
...
__name(handleAdminCategoryUpdate, "handleAdminCategoryUpdate");
async function handleAdminReviews({ env, request, url }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
...
async function handleAdminReviewDetail({ env, request, params }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
  const { id } = reviewIdParamSchema.parse(params);
...
__name(handleAdminReviewDetail, "handleAdminReviewDetail");
async function handleAdminComments({ env, request, url }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
...
__name(handleAdminComments, "handleAdminComments");
...
async function handleAdminUserDetail({ env, request, params }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
  const { userId } = userIdParamSchema.parse(params);
...
__name(handleAdminReports, "handleAdminReports");
async function handleAdminReportPatch({ env, request, params }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
  enforceRateLimit(request, env, user.id);
  const { id } = external_exports.object({ id: external_exports.string().uuid() }).parse(params);
...
__name(handleAdminReportPatch, "handleAdminReportPatch");
...
__name(handleAdminReviewStatus, "handleAdminReviewStatus");
...
__name(handleAdminReviewUpdate, "handleAdminReviewUpdate");
...
__name(handleAdminCommentUpdate, "handleAdminCommentUpdate");
...
__name(handleAdminCommentStatus, "handleAdminCommentStatus");
async function handleAdminUserUpdate({ env, request, params }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
  enforceRateLimit(request, env, user.id);
  const { userId } = userIdParamSchema.parse(params);
...
__name(handleAdminUserUpdate, "handleAdminUserUpdate");
async function handleAdminUserRole({ env, request, params }) {
  const user = await requireAuth(request, env);
  requireRole(user, "admin");
  enforceRateLimit(request, env, user.id);
  const { userId } = userIdParamSchema.parse(params);
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const handler = /* @__PURE__ */ __name(() => match.route.handler(context), "handler");
      const response = match.route.cacheTtl ? await cacheResponse(request, ctx, match.route.cacheTtl(parsedEnv2), handler) : await handler();
      const withNoStore = match.route.noStore ? new Response(response.body, response) : response;
      ...
      if (error instanceof HttpError) {
        ...
        const message = error && typeof error === "object" && "message" in error ? String(error.message) : "Internal Server Error";
        console.error(
          JSON.stringify({
            level: "error",
...
// .wrangler/tmp/bundle-LsDNLa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LsDNLa/middleware-loader.entry.ts
