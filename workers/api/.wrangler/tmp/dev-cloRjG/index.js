...
// .wrangler/tmp/bundle-6W7msv/strip-cf-connecting-ip-header.js
...
var categorySelect = "id, name, parent_id";
...
async function fetchSubcategories(env, parentId) {
  const supabase2 = getSupabaseClient(env);
...
var reviewDetailSelect = `
  ...
  status
`;
async function fetchPopularReviews(env, limit) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select(reviewListSelect).eq("status", "published").order("votes_up", { ascending: false }).order("views", { ascending: false }).limit(limit);
  if (error) {
    throw error;
  }
...
async function fetchLatestReviews(env, cursor, limit) {
  const supabase2 = getSupabaseClient(env);
  let query = supabase2.from("reviews").select(reviewListSelect).eq("status", "published").order("created_at", { ascending: false }).limit(limit);
  if (cursor) {
...
async function fetchReviews(env, options) {
  ...
  let query = supabase2.from("reviews").select(reviewListSelect, { count: "exact" });
  if (categoryId) {
...
async function fetchReviewBySlug(env, slug) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select(reviewDetailSelect).eq("slug", slug).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function fetchReviewMetaById(env, id) {
  const supabase2 = getSupabaseClient(env);
  const { data, error } = await supabase2.from("reviews").select("slug, category_id, status, profiles(username)").eq("id", id).maybeSingle();
  if (error) {
    throw error;
  }
  if (!data) {
...
async function fetchReviewsByUserComments(env, userId, page, pageSize) {
  const supabase2 = getSupabaseClient(env);
  ...
  const rows = data ?? [];
  const items = rows.map((row) => mapReviewRow(row));
...
async function fetchSavedReviews(env, userId, page, pageSize) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  ...
  const items = (data ?? []).map((row) => row.reviews).filter((review) => Boolean(review)).map((review) => mapReviewRow(review));
  return {
    items,
    pageInfo: buildPaginationInfo(page, pageSize, count ?? 0)
  };
...
async function fetchReviewsByUserId(env, userId, page, pageSize, statuses = ["published"]) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  let query = supabase2.from("reviews").select(reviewListSelect, { count: "exact" }).eq("user_id", userId).order("created_at", { ascending: false });
...
async function searchReviews(env, options) {
  ...
  const { data, error } = await supabase2.rpc("search_reviews", {
...
async function fetchSitemapReviews(env, page, pageSize) {
  const supabase2 = getSupabaseClient(env);
  const from = (page - 1) * pageSize;
  const to = from + pageSize - 1;
  const { data, error, count } = await supabase2.from("reviews").select("slug, created_at, updated_at", { count: "exact" }).eq("status", "published").order("created_at", { ascending: false }).range(from, to);
  if (error) {
    throw error;
  }
...
async function fetchSitemapReviewCount(env) {
  const supabase2 = getSupabaseClient(env);
  const { count, error } = await supabase2.from("reviews").select("id", { count: "exact", head: true }).eq("status", "published");
  if (error) {
    throw error;
  }
  return count ?? 0;
...
async function fetchSitemapCategories(env) {
  const supabase2 = getSupabaseClient(env);
...
async function handleSubcategories({ env, params }) {
...
async function handleReviewBySlug({ env, params }) {
...
async function handleSitemapCategoriesJson({ env }) {
...
// .wrangler/tmp/bundle-6W7msv/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6W7msv/middleware-loader.entry.ts
